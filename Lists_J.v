...
(** ただしこれを使うには、[coqc]を使って[Basics_J.v]をコンパイルし、[Basics_J.vo]を作成しておく必要があります。（これは、.javaファイルから.classファイルを作ったり、.cファイルから.oファイルを作ったりするのと同じことです。）

    あなたの環境でコードをコンパイルするには、以下の二つの方法があります。

     - CoqIDEで行う

         CoqIDEの"Compile"メニューから"Basics_J.v"を開き、"Compile" メニューの "Compile Buffer"をクリックする。
