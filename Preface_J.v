...
このコースの主な特徴は、ここで学ぶ"開発"というものが「形式的」で「機械的にチェックされた」ものであるということです。この文言はそのままCoqによる証明補助にあてはまります。またこのコースでは、Coqのインタラクティブモードを使い、ソースを1行1行追いながら動きを理解していきます。講義のほとんどはCoqで組み立てられ、Coq上で作業し演習するようデザインされています。
...
特に、帰納的に定義された集合や関係とその帰納的な証明は、コンピュータサイエンスのいたるところで見られます。このコースでは、帰納法をあらゆる角度から分析します。
  ...
  - 関数プログラミング：ソフトウェア開発者が持つべき道具のうち、重要性が特に増している
       ...
       - 特に永続的なデータ構造を用い、上体の変化を避けることで、並列プログラミングをシンプルなものとすることができます。
...
(** この資料の各章には、たくさんの練習問題がついています。"optional（任意）"と記されたり"recommended（推奨）"とされているものもあります。"任意"とされていない問題までやることで、その章で学ぶべきことを6～8時間（長い章でも）の学習で理解できるようになっています。
    ...
    練習問題についている"スターレーティング"には、以下のような意味があります。
       ...
       - 星一つ：多くの読者が1～2分でできる簡単な問題。"推奨"と明示しているものはありませんが、どちらかというと全て"推奨"とされるべきものです。読者は、この問題に取り組んで、このレベルの問題に慣れておくべきです。

       - 星二つ：　素直で簡単な問題（5～10分でできるでしょう）

       - 星三つ：　少し考えないといけない問題（15～30分ほどかかるでしょう）
