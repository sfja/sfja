...
(* $Date: 201f1-06-22 10:06:32 -0400 (Wed, 22 Jun 2011) $ *)
...
(** っここにあるいくつかの練習問題は、List.vにあったものと同じですが、多相性の練習になります。以下の定義を行い、証明を完成させなさい。 *)
...
(** このコースでこれ以降、、関数のオーバーライド（上書き）はよく登場しますが、この性質について多くを知る必要はありません。しかし、これらの性質を証明するには、さらにいくつかのCoqのタクティックを知らなければなりません。それが、この章の残りの部分の主なトピックになります。 *)
...
(** [inversion]のもう一つの側面を見てみましょう。以前にすでに証明済みのものですが、少し遠回りな書き方になっています。新しく追加され等号のせいで、少し等式に関する証明を追加する必要がありますし、これまでに出てきたタクティックを使う練習にもなります。 *)
...
(** 同様に、[apply L in H]というタクティックは、ある条件式[L] ([L1 -> L2]といった形の)を、コンテキストにある過程[H]に適用します。しかし普通のあ[apply]と異なるのは、[apply L in H]が、[H]が[L1]とマッチすることを調べ、それに成功したらそれを[L2]に書き換えることです。
